...
#if CONFIG_ESP_FACE_DETECT_ENABLED
    // FORCE ON DETECT ENGINE IF PIN 13 IS LOW
    if (digitalRead(START_STREAM_GPIO) == HIGH) {
        detection_enabled = 1;
    }
    ...
    if (fb->format == PIXFORMAT_RGB565
     ...
     && !recognition_enabled
        ...
        if (results.size() > 0) {
            ...
            detected = true;
#endif
            if (digitalRead(START_STREAM_GPIO) == HIGH || isStreaming == true) {
...
static esp_err_t stream_handler(httpd_req_t *req)
{
    ...
    char *part_buf[128];
#if CONFIG_ESP_FACE_DETECT_ENABLED
    // FORCE DETECTION ON IN BROADCAST STREAM LOOPS IF HANDSHAKE WIRE ACTIVE
    if (digitalRead(START_STREAM_GPIO) == HIGH) {
        detection_enabled = 1;
    }
    ...
    {
#if CONFIG_ESP_FACE_DETECT_ENABLED
        // Re-verify tracking line condition on every individual frame pass
        if (digitalRead(START_STREAM_GPIO) == HIGH) {
            detection_enabled = 1;
        }
                    ...
                    if (!out_buf) {
                        ...
                        if (!s) {
                            ...
                            if (results.size() > 0) {
                                ...
                                if (digitalRead(START_STREAM_GPIO) == HIGH || isStreaming == true) {
                                    auto face = results.front();
                                    int x_left   = (int)face.box[0];
                                    int y_top    = (int)face.box[1];
                                    int x_right  = (int)face.box[2];
                                    int y_bottom = (int)face.box[3];

                                    int box_w = x_right - x_left + 1;
                                    int box_h = y_bottom - y_top + 1;
                                    int x_center = x_left + (box_w / 2);
                                    int y_center = y_top + (box_h / 2);
...
#if CONFIG_ESP_FACE_DETECT_ENABLED
static void autonomous_tracking_task(void *pvParameters) {
#if TWO_STAGE
    HumanFaceDetectMSR01 s1(0.1F, 0.5F, 10, 0.2F);
    HumanFaceDetectMNP01 s2(0.5F, 0.3F, 5);
#else
    ...
    while (true) {
        ...
        if (!isStreaming && pin_state) {
            ...
            if (fb->width > 400) {
                // Frame too large for face detection, skip it
            ...
            if (fb->format == PIXFORMAT_RGB565) {
                ...
                if (out_buf) {
                    ...
                    if (s) {
#if TWO_STAGE
                        std::list<dl::detect::result_t> &candidates = s1.infer((uint8_t *)out_buf, {(int)fb->height, (int)fb->width, 3});
                        std::list<dl::detect::result_t> &results = s2.infer((uint8_t *)out_buf, {(int)fb->height, (int)fb->width, 3}, candidates);
#else
                    ...
                    Serial.println("DEBUG: malloc for RGB888 failed");
                }
            }
            if (!detected) {
                Serial.printf("FACE:-2,0,0\n");
...
{
    pinMode(START_STREAM_GPIO, INPUT_PULLDOWN);
#if CONFIG_ESP_FACE_DETECT_ENABLED
    xTaskCreatePinnedToCore(&autonomous_tracking_task, "auto_track", 8192, NULL, 5, NULL, 1);
#endif
